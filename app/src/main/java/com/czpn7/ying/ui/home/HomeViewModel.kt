    ...
    fun openHotItem(item: HotListItem) {
        ...
        viewModelScope.launch {
            ...
            val matched = searchRepository.search(item.title)
                .let { results ->
                    results.firstOrNull { normalize(it.primary.name) == normalizedTitle }
                        ?: results.firstOrNull()
                }
            ...
            if (matched == null) {
                uiState = uiState.copy(
                    resolvingItemId = null,
                    lookupMessage = "可播放源暂时没有匹配到《${item.title}》"
                )
                return@launch
            }
